...
namespace caffe {
    ...
    void Blob<Dtype>::Reshape(const int num, const int channels, const int height,
                              const int width) {
        /*将传入的各个通道参数形成一个vec数组,然后调用多态函数Reshape将shape赋给blob中的对应成员变量(blob.shape_)*/
        vector<int> shape(4);
        shape[0] = num;
        shape[1] = channels;
        shape[2] = height;
    ...
    void Blob<Dtype>::Reshape(const vector<int> &shape) {
        ...
        /*如果shape_data_没有初始化或者容量不够大,则重新申请一块内存*/
        if (!shape_data_ || shape_data_->size() < shape.size() * sizeof(int)) {
            shape_data_.reset(new SyncedMemory(shape.size() * sizeof(int)));
        }
        /*取出shape_data_中易变数据的内存地址,注意shape_data_与shape_的内容是相同的,区别在于shape_data_是自动状态转换机*/
        int *shape_data = static_cast<int *>(shape_data_->mutable_cpu_data());
        for (int i = 0; i < shape.size(); ++i) {
            CHECK_GE(shape[i], 0);/*检查有没有小于等于0的值*/
            if (count_ != 0) {
                /*判断shape有没有超出INT类型的范围,这里/count_是因为count_最终为H*C*H*W,在循环中会随着每个维度的增加而增加,
                 * 必须保证count_不能超过INT的取值范围,否则blob在取值的时候会发生越界*/
                CHECK_LE(shape[i], INT_MAX / count_) << "blob size exceeds INT_MAX";
            }
            count_ *= shape[i];/*计算总内存占用,随着维度阶乘,最终为H*C*H*W,即blob的元素容积*/
            shape_[i] = shape[i];/*将shape数据赋给blob的shape_成员*/
            shape_data[i] = shape[i];/*将shape数据放入负责同步C&GPU之间数据的shape_data_成员变量中*/
        }
        if (count_ > capacity_) {/*capacity为blob的容积,初始化时为0*/
            capacity_ = count_;
            /*对data和diff进行初始化,申请内存*/
...
// Blob<int> or Blob<unsigned int>.
    ...
    void Blob<Dtype>::CopyFrom(const Blob &source, bool copy_diff, bool reshape) {
        if (source.count() != count_ || source.shape() != shape_) {
            if (reshape) {
                ReshapeLike(source);/*拷贝source的shape到当前blob*/
            } else {
