...
// instead of duplicated here and in SoftMaxWithLossLayer
    ...
    void SigmoidCrossEntropyLossLayer<Dtype>::Backward_cpu(
        ...
        if (propagate_down[0]) {
            ...
            const Dtype *sigmoid_output_data = sigmoid_output_->cpu_data();/*sigmoid运算结果*/
            const Dtype *target = bottom[1]->cpu_data();/*target，即GT值*/
            Dtype *bottom_diff = bottom[0]->mutable_cpu_diff();/*保存本层的diff*/
            caffe_sub(count, sigmoid_output_data, target, bottom_diff);/*计算target与模型输出之间的差值，即为本层的diff*/
            ...
            // Scale down gradient
            Dtype loss_weight = top[0]->cpu_diff()[0] / normalizer_;/*如果有设定不同类别loss的权重则还需要乘权重*/
