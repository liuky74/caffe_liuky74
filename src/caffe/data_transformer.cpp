...
namespace caffe {
    ...
    void DataTransformer<Dtype>::Transform(
        ...
        NormalizedBBox crop_bbox;/*用于储存crop后的裁切坐标,已归一化到0~1之间*/
        /*crop剪切;镜像翻转;通道均值化*/
        Transform(datum, transformed_blob, &crop_bbox, do_mirror);
    ...
    void DataTransformer<Dtype>::TransformAnnotation(
            const AnnotatedDatum &anno_datum, /*包含了图像datum和label datum*/
            const bool do_resize,/*是否resize*/
            const NormalizedBBox &crop_bbox, /*裁切坐标,已归一化到0~1之间*/
            const bool do_mirror,/*是否镜面翻转*/
            RepeatedPtrField<AnnotationGroup> *transformed_anno_group_all/*储存转换后的label数据*/
            ) {
        ...
        if (anno_datum.type() == AnnotatedDatum_AnnotationType_BBOX) {
            ...
            for (int g = 0; g < anno_datum.annotation_group_size(); ++g) {
                const AnnotationGroup &anno_group = anno_datum.annotation_group(g);/*取出当前cls的GT BBOX*/
                AnnotationGroup transformed_anno_group;
                // Go through each Annotation.
                bool has_valid_annotation = false;
                for (int a = 0; a < anno_group.annotation_size(); ++a) {
                    const Annotation &anno = anno_group.annotation(a);/*挨个取出GT bbox*/
                    const NormalizedBBox &bbox = anno.bbox();/*GT bbox的box坐标*/
                    // Adjust bounding box annotation.
                    NormalizedBBox resize_bbox = bbox;
                    if (do_resize && param_.has_resize_param()) {/*如果图像经过resize,将bbox也进行对应的缩放*/
                    ...
                    if (ProjectBBox(crop_bbox, resize_bbox, &proj_bbox)) {/*由于图像经过crop和resize,所以也需要对GT label进行调整*/
                        has_valid_annotation = true;/*ProjectBBox返回True表示有可用的GT label*/
                        /*向transformed_anno_group中添加转换后的label*/
                        Annotation *transformed_anno = transformed_anno_group.add_annotation();/*添加一个元素,并且取出元素地址*/
                        transformed_anno->set_instance_id(anno.instance_id());/*写入样本id*/
                        NormalizedBBox *transformed_bbox = transformed_anno->mutable_bbox();/*取出bbox地址*/
                        transformed_bbox->CopyFrom(proj_bbox);/*将转换后的bbox拷贝进去*/
                ...
                // Save for output.
                if (has_valid_annotation) {/*当前分类下经过处理有可用的label,将其写入*/
