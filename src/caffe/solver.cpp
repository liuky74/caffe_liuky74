...
namespace caffe {
    ...
    template<typename Dtype>/*这个函数由用户实现，就是在有信号输入的时候该做什么处理，由用户来决定。*/
...
/*solver构造器会初始化一个net,回调函数,以及一个根solver解算器*/
    ...
    Solver<Dtype>::Solver(const SolverParameter &param, const Solver *root_solver)
        ...
        Init(param);/*通过各个子类调用父类时执行,初始化解算器*/
    }
    ...
    template<typename Dtype>
    void Solver<Dtype>::Init(const SolverParameter &param) {/*初始化解算器*/
        CHECK(Caffe::root_solver() || root_solver_)
        << "root_solver_ needs to be set for all non-root solvers";
        LOG_IF(INFO, Caffe::root_solver()) << "Initializing solver from parameters: "
                                           << std::endl << param.DebugString();/*输出配置*/
        param_ = param;
        /*检查平均loss不能是负数*/
        ...
        // Scaffolding code
        InitTrainNet();/*初始化训练模型*/
        if (Caffe::root_solver()) {
            InitTestNets();/*初始化测试模型*/
    ...
    template<typename Dtype>
    void Solver<Dtype>::InitTrainNet() {/*在Solver::Init函数中被调用,初始化模型训练相关*/
        const int num_train_nets = param_.has_net() + param_.has_net_param() +
                                   param_.has_train_net() + param_.has_train_net_param();
        const string &field_names = "net, net_param, train_net, train_net_param";
        /*检查有没有训练网络，如果没有训练网络则直接报错*/
        ...
        // precedence).
        /* 因为caffe里面网络有的是指定了专门训练用的，有的是指定了专门测试用的，有的没有指定就是通用的，
         * 通用的就是训练和测试都可以使用。 然后拷贝网络的状态到 NetState里面*/
        NetState net_state;
        net_state.set_phase(TRAIN);/*設置模型類型*/
        net_state.MergeFrom(net_param.state());
        net_state.MergeFrom(param_.train_state());
        net_param.mutable_state()->CopyFrom(net_state);
        if (Caffe::root_solver()) {
            net_.reset(new Net<Dtype>(net_param));/*在解算器初始化的过程中调用网络的初始化函数*/
        } else {
    ...
    void Solver<Dtype>::Step(int iters) {
        ...
        while (iter_ < stop_iter) {
            ...
            /* callbacks_, 定义了一个 Callback的类里面包含on_start和on_gradients_ready两个函数,
             * 由用户来负责定义,方便在Step执行中间进行一些操作,其中on_start在前向推导前执行,
             * 默认为空函数直接跳过*/
            ...
            for (int i = 0; i < param_.iter_size(); ++i) {
                loss += net_->ForwardBackward();/*前向推导得到loss*/
            }
            loss /= param_.iter_size();
            // average the loss across iterations for smoothed reporting 将迭代中的平均损失进行平均，以实现平滑
            UpdateSmoothedLoss(loss, start_iter, average_loss);
            /*如果需要显示结果,则提取输出blob*/
            if (display) {
                LOG_IF(INFO, Caffe::root_solver()) << "Iteration " << iter_
                                                   << ", loss = " << smoothed_loss_;
                const vector<Blob<Dtype> *> &result = net_->output_blobs();/*调出网络的输出blob*/
                int score_index = 0;
                /*提取result blob中的数据*/
            ...
            /*自定义回调类,包含on_start和on_gradients_ready两个方法,on_gradients_ready在前向推导执行后执行*/
            for (int i = 0; i < callbacks_.size(); ++i) {
    ...
    void Solver<Dtype>::Solve(const char *resume_file) {
        ...
        /*step即主循环函数*/
        Step(param_.max_iter() - iter_);/*根据迭代次数开始训练*/
        ...
        /* 优化完成后，如果需要,运行其他train和test网络以显示loss/output。
         * 与其余模式(test)不同，对于train网络，我们仅运行了前向遍历，
         * 因为我们已经更新了参数“max_iter”次-最终遍历仅用于显示损耗，这是在前向遍历中计算的。*/
    ...
    void Solver<Dtype>::CheckSnapshotWritePermissions() {
        /* 生成了一个tempfile把它写入这个地方看看，如果文件顺利生成的话就把他给删掉，
         * 如果有错误的话就抛出异常，提示不能写他，看看是不是没有权限写。
         * */
