...
// limitation that the data has to be contiguous in memory.
    template<typename Dtype>/*计算公式C = alpha*op( A )*op( B ) + beta*C */
    void caffe_cpu_gemm(const CBLAS_TRANSPOSE TransA,/* 矩阵A是否转置*/
                        const CBLAS_TRANSPOSE TransB, /* 矩阵B是否转置，在行主序下为了避免cache缺失，第二个矩阵通常会做转置*/
                        const int M,/* 输出结果C的row数，根据矩阵乘法法则也表示A的row数*/
                        const int N,/* 输出结果C的colum数，根据矩阵乘法法则也表示B的colum数*/
                        const int K,/* 表示 A的列数或B的行数（A的列数=B的行数），即为矩阵运算中被吃掉的维度*/
                        const Dtype alpha,/* 计算公式C = alpha*op( A )*op( B ) + beta*C中的两个参数值；*/
                        const Dtype *A, /*矩阵A的内存指针*/
                        const Dtype *B, /*矩阵B的内存指针*/
                        const Dtype beta, /* 计算公式C = alpha*op( A )*op( B ) + beta*C中的两个参数值；*/
                        Dtype *C /*矩阵C的内存指针*/
                        );

    template<typename Dtype>/*类似caffe_cpu_gemm，区别在于这个函数用于矩阵与常量值之间的运算*/
